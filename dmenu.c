...
read_Xresources(void) {
	...
	if ((xrm = XResourceManagerString(drw->dpy))) {
		...
		if (XrmGetResource(xdb, "dmenu.color8", "*", &type, &xval) == True) /* normal hi background color */
			colors[SchemeNormHighlight][ColBg] = strdup(xval.addr);
		if (XrmGetResource(xdb, "dmenu.color0", "*", &type, &xval) == True) /* normal hi foreground color */
			colors[SchemeNormHighlight][ColFg] = strdup(xval.addr);
		if (XrmGetResource(xdb, "dmenu.color0", "*", &type, &xval) == True) /* selected hi background color */
			colors[SchemeSelHighlight][ColBg] = strdup(xval.addr);
		if (XrmGetResource(xdb, "dmenu.color14", "*", &type, &xval) == True) /* selected hi foreground color */
